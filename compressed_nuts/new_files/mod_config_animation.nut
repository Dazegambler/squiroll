...
function Initialize(){this.action<-::menu.mod_config.weakref()
this.x<--1280
this.y<-0
this.visible<-true
local obj
...
this.item_key<-[]
this.item_pad<-[]
for(local i=0;i<10;i=++i){local _y=::menu.common.item_y+i*36
...
function Update(){local y=this.action.cursor.y
foreach(i,v in this.item){v.red=v.green=v.blue=y==i?1.00000000:0.50000000
}
foreach(v in this.item_pad){}
foreach(v in this.item_key){}
foreach(i,v in this.action.item){this.SetPad(this.item_pad[i],this.action.pad[v])
...
foreach(v in this.item){v.SetWorldTransform(mat)
}
foreach(v in this.item_pad){v.SetWorldTransform(mat)
}
foreach(v in this.item_key){v.SetWorldTransform(mat)
}
