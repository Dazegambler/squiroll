...
this.color_num<-[8,8]
this.icon<-[null,null]
function func_get_delay(){return 0}
function Initialize(){this.inst=null
this.inst_connect=null
this.return_code=-1
this.input_local=null
this.is_client=false
...
this.client_num=0
this.icon=[null,null]
this.func_get_delay=function(){return 0}}
function Terminate(){if(this.upnp_port>0){try{local ret=::UPnP.DeletePort(this.upnp_port,"UDP")}
catch(_e){}
this.upnp_port=0}
this.inst=null
this.inst_connect=null
this.is_disconnect=true}
function IsActive(){return this.inst!=null}
function IsPlaying(){if(this.inst==null){return false}
if(this.is_watch){return false}
return true}
function StartupServer(port,mode){if(::config.network.upnp){this.upnp_port=port
try{local ret=::UPnP.AddPort(port,port,"UDP")}
catch(_e){}}
else{this.upnp_port=0}
this.Initialize()
...
return false}
else{if(::config.network.allow_watch){table_dst.message="ready"
return false}
table_dst.message="watch"
return false}}
if(id>0){table_dst.message="busy"
return false}
...
return true}.bindenv(this)
mb_server.DisconnectChild=function(id){if(id==0){this.Disconnect()}}.bindenv(this)
mb_server.ReceiveFromChild=function(id,table){}.bindenv(this)
this.client_num=2
...
table_dst.is_watch<-true
return true}
mb_client.ConnectComplete=function(id,context,_reply_table){if(::LOBBY.GetNetworkState()!=::LOBBY.CLOSED){::LOBBY.Close()}
if(this.inst){return}
...
if(table.message=="watch"){this.return_code=3}
if(table.message=="ready"){this.return_code=4}}}.bindenv(this)
mb_client.DisconnectParent=function(){if(this.is_parent_vs){local t={}
t.message<-"end_vs"
for(local i=0;i<this.client_num;i++){this.inst.SendToChild(i,t)}
this.Disconnect()
return}
if(this.is_watch&&this.inst){this.inst.Reconnect()
return}}.bindenv(this)
...
function Disconnect(scene=true){if(this.is_disconnect){return}
if(!::network.IsActive()){return}
this.is_disconnect=true
if(scene){::loop.Fade(function(){if(::network.IsActive()){::network.Terminate()
::loop.End(::menu.network)}})}
else{::network.Terminate()}
return}
function GetDelay(){return this.func_get_delay()}
function BeginStreaming(){this.inst.BeginStreaming()}
function EndStreaming(){this.inst.EndStreaming()}
function BeginStreamingPlay(func_begin,func_end){this.inst.BeginStreamingPlay(func_begin,func_end)}
function IsEnableStreamingBuffer(){return this.inst.StreamingPlay()}
