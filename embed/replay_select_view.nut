function Initialize( _action )
{
	...
	function func_active()
	{
		foreach( v in this.item )
		{
			foreach( t in v.obj )
			{
				t.ConnectRenderSlot(::graphics.slot.overlay, 100000);
			}
		}
	}
...
function Update()
{
	...
	if (this.action.current_file)
	{
		...
		foreach( i, v in this.page[this.action.cursor_page.val].item )
		{
			...
			{
				obj.red = obj.green = obj.blue = i == this.action.cursor.val ? 1 : 0.5;
			}
		}
	}
...
function CreateDir( t, obj )
{
	...
	{
		obj.red = obj.green = obj.blue = 0.5;
	}
...
function CreateFile( t, obj )
{
	...
	if (obj.loading > 0)
	{
		text.ConnectRenderSlot(::graphics.slot.overlay, 100000);
	}
	else
	{
