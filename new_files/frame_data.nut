    ...
    local display = {
        ...
        flag_state = {
            ...
            function Render(data) {
                ...
                if (data.flag_state & 0x20) flags += "normal cancel,"; // 32
                ...
                if (data.flag_state & 0x400) flags += "bullet whiff,"; // 1024
        ...
        function Reset() {
            ...
            foreach(k in ["frame_data","flag_state","flag_attack"]){
                ...
                text.ConnectRenderSlot(::graphics.slot.front,1);
            }
        }
        ...
        function Update() {
            ...
            if(::setting.frame_data.enabled){
                local total = ::setting.frame_data.GetFrameCount(current);
                if (this.previous_data && total == this.previous_data.frame_total){
                    data.count = this.previous_data.count;
                    data.frames = this.previous_data.frames;
                    data.cancels = this.previous_data.cancels;
                    data.armor = this.previous_data.armor;
                    data.frame_total = this.previous_data.frame_total;
                }
            ...
            this.framebar.Render(data);
            if (this.full) {
                this.frame_data.Render(data);
                this.flag_state.Render(data);
                this.flag_attack.Render(data);
            }else {
