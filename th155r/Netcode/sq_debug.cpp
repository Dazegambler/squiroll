...
static void print_stack_top_value(HSQUIRRELVM v, int depth) {
    ...
    switch (uint32_t val_type = _RAW_TYPE(sq_gettype(v, -1))) {
        ...
        case _RT_CLASS: case _RT_INSTANCE:
            ...
            sq_pushstring(v, _SC("_typename"), -1);
            if (SQ_SUCCEEDED(sq_get(v, -2))){
                if (SQ_SUCCEEDED(sq_getstring(v, -1,&classname))){
                    log_printf(val_type == _RT_CLASS ? "Class (%s type)" : "Instance (%s type)", classname);
                    
                } else {
                    log_printf(val_type == _RT_CLASS ? "Class (unknown type)" : "Instance (unknown type)");
                }
                sq_pop(v, 1);
            }else{
                sq_tostring(v, -1);
